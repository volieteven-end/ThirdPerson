...
2. 在 Manny/Quinn Simple 网格及共享骨架中补入 `hand_r → sword_motion` 控制骨，将其平移重定向模式设为 Animation。根据原动画武器相对左右握点的变换，适配 UE5 的手部位置，连续烘焙换手和翻剑运动。左右手旋转轴从源/目标对应姿势校准，不能直接把 `HandGrip_L` 的插槽旋转当成原版握剑方向。
...
2026-09-13 朝向修正：攻击 2 第 32 帧原本应水平挥剑，未经校准的左手插槽轴使它竖直向上。现在将旋转映射与位置映射分开处理，左右手均保留原版武器的旋转运动。Python 编辑器入口 `unreal.SwordMotionAssetTools.build_sword_motion(rotation_only=True)` 只更新已有武器轨道的旋转，不更改其位置/缩放，也不保存或旋转骨架、网格和插槽。此次使用该模式修复；逐帧检查非旋转数据差值为 0。
...
- `ThirdPerson.SwordMotion.SourceOrientation`：在 Manny 和 Quinn 上逐帧对照原始 UE4 动画的完整旋转及剑刃方向，单独检查攻击 2 第 32 帧不再竖剑；不能仅用“压缩数据等于烘焙数据”判断方向正确。
