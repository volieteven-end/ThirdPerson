...
void UTutorialCourse::PopulateDefaults(UTutorialCourse& C)
{
    ...
    auto& Move = Lesson(TEXT("Movement"), TEXT("01  入营 · 移动"), TEXT("沿金色石路前进，完成疾跑和跳跃。空格是闪避 / 疾跑，F 才是跳跃。"));
    ...
    Goal(Move, ETutorialSignal::SprintMarker, TEXT("持续疾跑穿过前方金色路标"), TEXT("移动 + 长按空格  疾跑"), TEXT("Sprint"));
    Goal(Move, ETutorialSignal::JumpLanding, TEXT("按 F 跳过矮墙，落在金色平台上"), TEXT("F  跳跃"), TEXT("Jump"));
    ...
    auto& Dodge = Lesson(TEXT("Dodge"), TEXT("03  身法 · 闪避"), TEXT("短按空格立即松开；按住会变成疾跑。先向前，再向侧面闪避。"));
    Goal(Dodge, ETutorialSignal::ForwardDodge, TEXT("朝前方金圈闪避，并进入圈内"), TEXT("W + 短按空格"), TEXT("DodgeForward"));
    Goal(Dodge, ETutorialSignal::SideDodge, TEXT("保持面朝前方，向右侧金圈闪避"), TEXT("D + 短按空格"), TEXT("DodgeSide"));
    ...
    Goal(Combat, ETutorialSignal::EnemyDefeated, TEXT("击败第一名陪练，再迎战第二名"), TEXT("左键攻击  ·  空格闪避  ·  右键防御  ·  Q 血瓶"), TEXT("CombatTarget"), 2);
    ...
    Goal(Air, ETutorialSignal::AirPair, TEXT("腾空后依次打出两段空中攻击"), TEXT("F 跳跃 / 升龙后，连续左键"));
