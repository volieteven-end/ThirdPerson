...
$Run = Join-Path $ProjectRoot ('Saved\Automation\SwordActionVerification_' + (Get-Date -Format 'yyyyMMdd_HHmmss'))
...
try {
    ...
    foreach ($Mode in $Modes) {
        ...
        else { $Arguments += @('-RenderOffscreen', '-SwordVisualAudit', '-ResX=1280', '-ResY=720') }
        ...
        if ($Code -ne 0 -or $Report.failed -ne 0 -or $Bad.Count -ne 0 -or $Passed -lt 31) {
