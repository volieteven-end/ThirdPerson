...
本文件保留实施前的设计规格与资源核对事实。当前 C++ 实现、经实测校准的参数和运行方法见 `CountessBoss实现说明.md`，构建与 PIE 证据见 `CountessBoss验收记录.md`；不以设计初值替代实际验收结果。
...
按当前工程开发约定在原项目中增量修改、构建和测试，使用既有 Git 历史保存版本；不建立工程副本、额外哈希清单或回滚测试工程。原 Paragon 资源不就地改写，新增 Boss 资产集中管理。

实施结果以源代码、必要 UE 资产、相关测试记录和 Git 提交交付，通过后推送既有远端。需要撤销已提交版本时使用 Git 的撤销提交，保留其他用户修改和历史。只有实际完成构建和 PIE 后，才把上述验收项标为通过。当前实现结构与使用方法见 `CountessBoss实现说明.md`。
