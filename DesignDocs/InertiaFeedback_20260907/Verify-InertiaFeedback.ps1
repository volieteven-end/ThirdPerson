...
$Run = Join-Path $ProjectRoot ('Saved\Automation\InertiaFeedbackVerification_' + (Get-Date -Format 'yyyyMMdd_HHmmss'))
...
try {
    ...
    foreach ($Mode in $Modes) {
        ...
        $Arguments = @($Project, '/Engine/Maps/Entry', '-ExecCmds=r.MotionBlurQuality 0, Automation RunTests ThirdPerson',
        ...
        else { $Arguments += @('-RenderOffscreen', '-InertiaVisualAudit', '-ResX=1280', '-ResY=720') }
        ...
        if (-not ($Report.tests.fullTestPath -contains 'ThirdPerson.Feedback.PIE.InertiaAndArrow')) { throw 'Inertia regression test was not discovered' }
        ...
        if ($Code -ne 0 -or $Report.failed -ne 0 -or $Bad.Count -ne 0 -or $Passed -lt 35) {
            throw "$Mode validation failed: exit=$Code passed=$Passed failed=$($Report.failed) nonSuccess=$($Bad.Count); $Index"
        }
        foreach ($Trace in @('movement_trace.csv')) {
            $Source = Join-Path $ProjectRoot ('Saved\InertiaFeedback\' + $Trace)
